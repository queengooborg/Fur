...
def language():
	global lang, msgs, langversneeded
	wait=True
	wait2=True
	while wait:
		while wait2:
			...
			else: output("Invalid option/Opcion incorrecto/L'option invalide")
		if lang == "English":
			with open('english.lang', 'rb') as handle: msgs = pickle.load(handle)
		else: output("Language File Version Incompatible/Version del Archivo del Idioma Incompatible/Version de L'archive du Language Incompatible")

#Function that prints the messages
def output(msg, dict=True, newline=True, noscroll=False, addon=None, addonfromdict=True, modifier="normal", r=0):
...
def quit(msg, nosave=False):
	global pc, devplayer
	mailto = outpuut(13, r=1)
	if nosave: pass
	else: save()
	color('red')
	if ios and not devplayer: notification.schedule(output(14, r=1), 15, 'Beep', mailto)
	if msg: sys.exit(msg)
	else: sys.exit(0)
...
def getCommand(sentence):
	...
	if len(words) > 2:
		if words[2] == output(129, r=1) or words[2] == output(130, r=1):
			if len(words) > 3: cmnd['using'] = words[3]
			else:
				output(133, addon=[verb, noun])
				return None
...
class Item(object):
	...
	def examine(self):
		if self.description: output(self.description)
		else: output(132, addon=self.name)
...
class Food(Item):
	...
	def eat(self):
		if self.poison:
			output(133, addon=self.name)
			quit(134, addon=self.name)
		else: output(135, addon=self.name)
...
class Drink(Item):
	...
	def drink(self):
		if self.poison:
			output(136, addon=self.name)
			quit(137, addon=self.name)
		else: output(135, addon=self.name)
...
class Character(object):
	def __init__(self, location, name, last, species, gender, level, life=100):
		...
		if self.species:
			self.attk = 25
			self.dfns = 25
			self.attacks = [Attack(output(143, r=1), output(144, r=1), 20, 0, output(145, r=1))]
		...
		def get_var(var):
			if var == 'all': return output(138, r=1, addon=[self.name, self.last, self.gender, self.species, self.inventory, self.location, self.level, self.attacks])
			else:
...
class Room(object):
	...
	def describe(self):
		output(self.name)
		for i in self.items:
			output(140, addon=i.name)
			time.sleep(0.1)
		for d in self.doors:
			output(141, addon=d.direction)
			time.sleep(0.1)
...
class Door(object):
	...
	def __str__(self):
		return output(142, r=1, addon=[self.direction,self.room])
...
def mainmenu():
	...
	scroll = temp
	time.sleep(3)
	if ios: temp = output(19, r=1)
	else: temp = output(20, r=1)
	getInput.alert(output(40, r=1, addon=temp))
	#print available choices and wait for the user to pick a valid choice
	not_chosen=True
	wait_opt=True
	while not_chosen:
		output('')
		choice = getInput.choice(output(1, r=1),[output(42, r=1),output(43, r=1, addon=41),output(44, r=1),output(17, r=1)], window=True)
		output('')
		if choice==1: not_chosen=False
		elif choice==2:
			getInput.alert(output(52, r=1))
			continue
			i = load()
			if i and devplayer: gameplay()
			elif not i: continue
			elif not devplayer: quit(output(53, r=1), nosave=True)
		elif choice==3:
			wait_opt = True
			while wait_opt:
				choice_opt = getInput.choice(output(1, r=1),[output(45, r=1, addon=scrollspeed),output(46, r=1, addon=41),output(47, r=1),output(146, r=1),output(18, r=1)])
				if choice_opt == 1:
					if scrollspeed==output(23, r=1):
						scrollspeed=output(21, r=1)
						scroll=0.03
					elif scrollspeed==output(21, r=1):
						scrollspeed=output(22, r=1)
						scroll=0.01
					else:
						scrollspeed=output(23, r=1)
						scroll=0.05
				elif choice_opt==2:
					getInput.alert(output(52, r=1))
					continue
					if not annoy:
						getInput.alert(output(48, r=1, addon=50))
						annoy = True
					else:
						getInput.alert(output(48, r=1, addon=51))
						annoy = False
				elif choice_opt==3:
					if not devplayer:
						getInput.alert(output(49, r=1, addon=50))
						devplayer = True
					else:
						getInput.alert(output(49, r=1, addon=51))
						devplayer = False
		...
		elif choice==0 or choice==4: quit(output(54, r=1), nosave=True)
		else: output(55)
...
def start():
	...
	waiting=True
	while waiting:
		temp = getInput.choice(output(56, r=1), [output(24, r=1), output(27, r=1)])
		if temp == 1:
			gender=output(24, r=1, modifier="lower")
			frnd_gender=output(27, r=1, modifier="lower")
			frnd_gndrpn=output(29, r=1)
			waiting=False
		elif temp == 2:
			gender=output(27, r=1, modifier="lower")
			frnd_gender=output(24, r=1, modifier="lower")
			frnd_gndrpn=output(26, r=1)
			waiting=False
		else:
			output(57)
	waiting = True
	while waiting:
		temp = getInput.text(output(58, r=1))
		...
		try:
			...
			if temp:
				temp = temp[0]
				while waiting:
					temp2 = getInput.text(output(59, r=1, addon=temp))
					temp2 = temp2.title()
					if temp2:
						player_name = temp
						player_last = temp2
	...
	output(61, addon=[player_name, player_last])
	i = 0
	time.sleep(2)
	while i != 3:
		i += 1
	...
	time.sleep(2)
	output(63)
	choice = getInput.choice(output(64, r=1), species)-1
	if choice == -1: quit(output(54, r=1), nosave=True)
	player_species = species[choice]
	output(65, addon=[player_species,player_species])
	time.sleep(4)
...
def init():
	global pc, ios, lang, scrollspeed, annoy, devplayer, msgs
